...
class DirectoryTree(QTreeWidget):
    ...
    def init_ui(self):
        ...
        self.setStyleSheet(f"""
            ...
            QTreeWidget::branch:closed:has-children:has-siblings {{
                border-image: none;
                image: url(data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMiIgaGVpZ2h0PSIxMiIgdmlld0JveD0iMCAwIDI0IDI0IiBmaWxsPSJub25lIiBzdHJva2U9IiM5YmE0YjQiIHN0cm9rZS13aWR0aD0iMiI+PHBhdGggZD0iTTkgMThsNi02LTYtNiIvPjwvc3ZnPg==);
            }}
            
            QTreeWidget::branch:open:has-children:!has-siblings,
            QTreeWidget::branch:open:has-children:has-siblings {{
                border-image: none;
                image: url(data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMiIgaGVpZ2h0PSIxMiIgdmlld0JveD0iMCAwIDI0IDI0IiBmaWxsPSJub25lIiBzdHJva2U9IiM5YmE0YjQiIHN0cm9rZS13aWR0aD0iMiI+PHBhdGggZD0iTTYgOWw2IDYgNi02Ii8+PC9zdmc+);
            }}
    ...
    def refresh(self):
        ...
        root_item.setData(0, Qt.ItemDataRole.UserRole + 1, True)  # is_directory
        
        self.addTopLevelItem(root_item)
        root_item.setExpanded(True)
        
        # Populate children and expand all
        self.populate_item(root_item, self.root_path)
    ...
    def expand_all_items(self, item):
        ...
        if not item_name.endswith('.assets'):
            item.setExpanded(True)
            for i in range(item.childCount()):
                child = item.child(i)
                self.expand_all_items(child)
    
    def populate_item(self, parent_item, parent_path):
        """Populate tree items"""
        try:
            items = []
            for item_name in os.listdir(parent_path):
                ...
                # Filter: show .md files, image files, and directories
                ...
                if is_dir and not item_name.endswith('.assets'):
                    # Check if it contains .md files or is the docs folder
                    has_content = any(
            ...
            for item, item_path, is_dir, name in items:
                ...
                if is_dir:
                    if not name.endswith('.assets'):
                        item.setExpanded(True)
                    self.populate_item(item, item_path)
    ...
    def get_current_path(self):
        ...
        if current_item:
            ...
            if is_dir:
                ...
                # Return parent directory for files
